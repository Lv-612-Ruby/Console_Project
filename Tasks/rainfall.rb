...
def mean(town, strng)
  return -1 if (numbers=extract_data(town, strng)).nil? #here is a data check
  numbers.sum/12 # get the avarage value of rainfall for the year
end

def variance(town, strng)
 return -1 if (numbers=extract_data(town, strng)).nil? #here is a data check
 mean = numbers.sum/12
 variance =numbers.map {|x| (x-mean)**2} .sum/12 # get the variance value of rainfall for the year
end

def extract_data(town,strng) #this function extracts data from hashes
 data_hash = strng.split("\n").map{ |str| str.split(":") }.to_h
 return nil if data_hash[town].nil? #data check
 numbers = data_hash[town].scan(/\d+.\d+/).map(&:to_f)#check for digit, and reduces the value to float and we see numbers after comma.
end
...
puts mean("London", data) #51.19(9999999999996) 
puts variance("London", data) #57.42(833333333374)
>>>>>>> 6f7e2cdad4806d285c835f017303027a443313a3
